...
| Message | Level | Attributes | When |
| --- | --- | --- | --- |
| `get records failed; backing off` | Warn | `shard`, `consecutive_failures`, `backoff`, `error` | A retryable GetRecords failure (throttling, 5xx, network) — the pass backs off in-place (500ms doubling to a 10s cap, plus up to 50% jitter so throttled shard readers don't retry in lockstep) and retries the same iterator instead of stopping the consumer. |
| `records skipped after handler failure` | Warn | `shard`, `handler`, `records`, `first_sequence`, `last_sequence`, `attempts`, `error` | Explicit `skip` policy intentionally dropped a failed record (record mode) or whole page (batch mode) after retries were exhausted. The page can then be checkpointed past the dropped data. `first_sequence`/`last_sequence` bound the dropped range so it stays identifiable for replay or audit — this line is the only record of the loss. This Warn is emitted only when `WithLogger` enables logging; the default logger discards it. |
