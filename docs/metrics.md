...
| `kinesis_consumer.heartbeat_failures` | failures | `stream`, `consumer_group` | A worker-liveness heartbeat send fails (live context). Sustained failures mean peers will treat this worker as dead and claim its shards away. |
| `kinesis_consumer.rebalance_pass_failures` | failures | `stream`, `consumer_group` | A rebalance pass returns an error and is skipped until the next tick (live context). Shard-sync failures are not counted here — they stop the consumer. |
