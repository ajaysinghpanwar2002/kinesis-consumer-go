...
| --- | --- | --- | --- |
| `kinesis_consumer.records_processed` | records | `stream`, `consumer_group`, `shard`, `handler` | A record handler succeeds, or a batch handler succeeds (adds the batch size). Records handled by skip or DLQ policy are excluded. In [explicit mode](explicit-processing.md) it counts the deliveries of a successful handler call, which is delivery, not acknowledgment or persistence. |
...
| `kinesis_consumer.checkpoint_failures` | failures | `stream`, `consumer_group`, `shard` | Any regular or shard-completion checkpoint save attempt fails, counted per attempt: saves retry up to the configured retry attempts with backoff, so one save that recovers on a later attempt still counts its earlier failures. A lost lease and unusable recovery state are excluded — neither is retried, and they count as `lease_lost` and `recovery_failures` instead. |
| `kinesis_consumer.recovery_failures` | failures | `stream`, `consumer_group`, `shard` | A fenced session reports unusable recovery state (`checkpoint.ErrRecoveryState`): missing or inconsistent metadata, an invalid recovery value, an unverifiable or trimmed sequence anchor, or observed progress regression. The shard halts and the error surfaces from `Start`; the consumer never re-anchors itself. See [fenced consumer recovery](fenced-recovery.md). |
...
| `kinesis_consumer.lease_lost` | leases | `stream`, `consumer_group`, `shard` | A renewal, a fenced recovery read, or a fenced progress write reports `ErrNotOwned` because a peer claimed the shard. The affected worker stops cleanly without releasing the peer-owned lease; other workers and the consumer run remain live. |
