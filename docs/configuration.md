...
| `WithInFlightLimits` | `InFlightLimits` | disabled for automatic handlers, always on in explicit mode | [Bound admitted records/bytes and fetched/staged pages](in-flight-limits.md); split batches into ordered prefixes. Zero fields select documented defaults. | All fields `>= 0` |
| `WithShardConcurrency` | `concurrency int` | `1` | Concurrent record-handler calls within one shard page. `> 1` improves throughput but breaks strict per-shard ordering. Automatic record handlers only; rejected in explicit mode, where deliveries are already acknowledged concurrently. | `>= 1` |
...
| `WithBatchHandler` | `handler BatchHandlerFunc` | none | Switches to one handler call per GetRecords page (or capacity-limited prefix) instead of per record. Mutually exclusive with the positional record handler in `New`: pass one or the other — the positional handler must be nil when this is set, and `New` errors if both are provided. | non-nil |
| `WithExplicitHandler` | `handler func(context.Context, Delivery) error` | none | Switches to [explicit acknowledgment](explicit-processing.md): the handler receives a `Delivery` and returning does not acknowledge it. Requires a fenced checkpoint store paired with the lease manager, and rejects `WithShardConcurrency`. Admission limits are always on; without `WithInFlightLimits` the documented explicit defaults apply. | non-nil; exactly one handler mode, with a nil positional handler |
| `WithExplicitBatchHandler` | `handler func(context.Context, []Delivery) error` | none | Explicit acknowledgment with one call per capacity-limited prefix. Each `Delivery` carries its own handle and is acknowledged independently. Same requirements as `WithExplicitHandler`. | non-nil; exactly one handler mode, with a nil positional handler |
| `WithCheckpointInterval` | `interval time.Duration` | `1s` | How often explicit mode flushes the contiguous acknowledged prefix, in addition to the `checkpointEvery` count trigger, drains, and shard completion. Rejected for automatic handlers, which flush by count. | `> 0`; explicit mode only |
| `WithHeartbeat` | `interval, ttl time.Duration` | `5s, 20s` | Worker liveness heartbeat cadence and lease/worker expiration lifetime. Failed heartbeat sends are survivable while the indexed entry from the last successful send is live; once failures persist to `ttl - interval` past that success, `Start` returns the causal error wrapped in `ErrHeartbeatStale` — one full interval before peers can observe the entry as expired and claim this worker's shards away. Heartbeat health (consecutive failures, last success, last error) is exposed via `Consumer.Health()`. | both whole-millisecond values `>= 1ms`; `ttl >= 3x interval` (enforced; the default is 4x); `ttl + interval` must fit in `time.Duration`. The 3x floor guarantees a renew/heartbeat call — which can run a full interval and is only checked for staleness after it returns — still completes before the `ttl - interval` safety deadline, so a lease cannot expire mid-call and let a peer reclaim the shard. A whole-millisecond value keeps Valkey from truncating the configured lifetime, and the `ttl + interval` bound keeps the lease watchdog deadline from overflowing. |
...
| `WithGracefulDrain` | `timeout time.Duration` | off | On ctx cancel, workers finish in-flight work, checkpoint, and release leases before `Start` returns. In explicit mode the drain additionally waits for every admitted delivery to be acknowledged; see the [shutdown order](explicit-processing.md#shutdown). `0` waits indefinitely. At a nonzero deadline, remaining workers are signaled to stop and `Start` returns `ErrDrainTimeout` without joining an uncooperative callback. Drains longer than `heartbeatTTL` are safe: the worker keeps heartbeating for the whole drain, and a shard a peer claims mid-drain counts as drained (not an error). One worker's failure lets the others finish before the deadline or error is returned. | `timeout >= 0` |
